<x-app-layout>
    ...
    <div class="bg-[#E8E4DC] min-h-screen w-full">
        ...
        <div class="max-w-3xl mx-auto py-12 px-4">
            ...
            {{-- Navigation --}}
            <a href="{{ route('trips.show', $itinerary->trip) }}" class="inline-flex items-center text-slate-500 hover:text-slate-700 mb-8 transition-colors">
            ...
            <div class="bg-white rounded-lg shadow-sm p-8 md:p-12">
                ...
                <form action="{{ route('itineraries.update', [$trip, $itinerary]) }}" method="POST" class="space-y-10">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                id="date-picker"
                                name="date"
                                value="{{ old('date', $itinerary->date ? \Carbon\Carbon::parse($itinerary->date)->format('Y-m-d') : '') }}"
                                ...
                                @for ($hour = 0; $hour < 24; $hour++)
                                    @foreach ([0, 15, 30, 45] as $minute)
                                        @php
                                            ...
                                            $isSelected = old('time', $itinerary->time ?? '') === $timeValue;
                                        @endphp
                                        <option value="{{ $timeValue }}" @selected($isSelected)>
                                            {{ $displayTime }}
                                        </option>
                                    @endforeach
                                @endfor
                        ...
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title', $itinerary->title) }}" placeholder="What's the plan?"
                                class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 focus:ring-2 focus:ring-slate-400 focus:bg-white transition-all outline-none">
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="note" rows="4" placeholder="Optional details..."
                                class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 focus:ring-2 focus:ring-slate-400 focus:bg-white transition-all outline-none">{{ old('note', $itinerary->note) }}</textarea>
                        </div>

                        {{-- Fee --}}
                        <div>
                            <label class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block">Fee (¥)</label>
                            <input type="number" name="cost" value="{{ old('cost', $itinerary->cost) }}" placeholder="0"
                                class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 focus:ring-2 focus:ring-slate-400 focus:bg-white transition-all outline-none">
                        </div>
                    </div>
    ...
    <script>
        flatpickr("#date-picker", {
            ...
            altInput: true,
            altFormat: "F j, Y",
            minDate: "{{ $itinerary->trip->start_date }}",
            maxDate: "{{ $itinerary->trip->end_date }}",
            defaultDate: "{{ old('date', $itinerary->date ? \Carbon\Carbon::parse($itinerary->date)->format('Y-m-d') : '') }}",
